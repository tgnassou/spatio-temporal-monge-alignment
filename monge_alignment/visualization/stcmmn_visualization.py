...
X_transform = ma.fit_transform(np.concatenate(X_all))
X_concat = [np.concatenate(X[i], axis=-1) for i in range(len(X))]
H = ma.compute_filter(X_concat)
# H = ma.compute_filter(X)

barycenter = ma.barycenter
# %%
freqs = scipy.signal.welch(X_concat[0][0], fs=100, nperseg=filter_size)[0]
psd = [ma._get_csd(X_concat[i]) for i in range(len(X_concat))]
X_transform_concat = [
    np.concatenate(X_transform[i], axis=-1) for i in range(len(X_transform))
]
psd_transform = [
    ma._get_csd(X_transform_concat[i])
